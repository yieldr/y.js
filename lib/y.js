...
(function (window) {
    ...
    y.track_form = function(element,inputelement){
        ...
        if(document.querySelector(element)!== null){
            var formvalue = function(inputelement){
                    ...
                    value = ( val === undefined ) ? "novalue" : val );

                    return value;
            }
            document.querySelector(element).addEventListener("submit", function(){
    ...
    y.track = function () {
        var sessionStorage = window.sessionStorage,
            ...
            ufq = cookie.get('yufq'),
            sfq = sessionStorage.getItem('ysfq');
            sessnr++,sfq++,ufq++, uer = ufq / sessnr;

        if (trafficSource === host) {
            trafficType = 'self';
        }

        cookie.set('yufq', ufq, 30, host);
        cookie.set('yuer', uer, 30, host);
        cookie.set('yhist', history.split(cookie.get('yhist'), '|').add(trafficSource).join('|'), 30, host);
        
        if (!sessid) {
            sessid = session();
            sesstime = 1;
            cookie.set('sessnr', sessnr, 30, host);
            sessionStorage.setItem('ysess', sessid);
            sessionStorage.setItem('ysesst', Math.floor(Date.now() / 1000));
            ...
            sesstime =  Math.floor(Date.now() / 1000) - sessionStorage.getItem('ysesst');
        }

        sessionStorage.setItem('ysfq', sfq);
        var ser = sfq / sesstime;
        cookie.set('yser', ser, 30, host);
