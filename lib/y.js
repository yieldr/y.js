...
(function (window) {
    ...
    var YieldrUtil = function () { };

    YieldrUtil.prototype.something = function () {};

    console.log(new YieldrUtil());

    /**
     * Yieldr Tracking Object
     */
    var Yieldr = function () { };

    /**
     * Callback is used to process the servers response in the form of JSON-P.
     *
    ...
    /**
     * Fire triggers an HTTP request to the remote server by placing a script
     * tag with the servers URL as the source.
     *
     * The params passed to this function are used as URL query parameters which
     * in turn are used by the server to segment a user.
     *
     * @param  {Object} params
    ...
    var url = (function () {
        ...
        /**
         * Extracts Google Analytics utm_* parameters from the query parameters
         * and unsets them from the query object.
         *
         * @param  {Object} query This will have its utm_* params removed.
         * @return {Object}
         */
        function utm(query) {
            var params = {};
            forEach(query, function (key, value) {
                if (key.indexOf('q_utm_') === 0) {
                    params[key] = value;
                    delete query[key];
                }
            })
     ...
     * @param {Function} callback
     */
    Yieldr.prototype.elem = function (selector, event, callback, attributes) {
        ...
        var attach = function (element, event) {
            addEvent(element, event, function (e) {
                if (!e.done) {
                    e.preventDefault();
                    var attrs = {};
                    for (var i = 0; i < attributes.length; i++) {
                        attrs['attr_' + attributes[i]] = element.getAttribute(attributes[i]);
                    }
                    var params = merge([
                        y.data,
     ...
     * #param {Boolean} all
     */
    // Yieldr.prototype.dl = function (object, mapping, all) {
    //     if (!mapping) {
    //         mapping = {};
    //     }
     ...
     * @type {String}
     */
    Yieldr.domain = Yieldr.domain || 'y.254a.com';
    ...
    (function () {
        ...
        var document = window.document,
            alias = window.YieldrTrackingObject || 'y',
            y = window[alias];

        var yieldr = new Yieldr();

        for (var i = 0; i < y.length; i++) {
            var fn = y[i].shift();
            if (yieldr[fn]) {
                yieldr[fn].call(null, y[i]);
            }
            delete y[i];
        };
