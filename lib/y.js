...
(function (window) {
    ...
    y.elem = function (args) {
        ...
        var selector = args[0],
            event = args[1],
            name = args[2] || selector + ':' + event,
            elements = document.querySelectorAll(selector);
        var trigger = function () {
            y.fire(merge([ y.data, { event: event, name: escape(name) } ]));
        };
    ...
    y.track = function () {
        ...
        // Session.
        var sessid = session.get('_yldr_session');
        if (!sessid) {
            sessid = uuid();
            session.set('_yldr_session', sessid);
            session.set('_yldr_traffic_source', traffic[0]);
