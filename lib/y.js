...
(function (window) {
     ...
     */
    window[yieldr].callback = function (response) {
     ...
     * @param  {Object} params
     */
    window[yieldr].fire = function (params) {
        var pairs = [];
        ...
        element.src = document.location.protocol + '//n.254a.com/pixel' + '?' + pairs.join('&');
    ...
    window[yieldr].track = function () {
        ...
        var data = merge([window[yieldr].data, params]);
        data.sessid = sessid;
        ...
        if (window[yieldr].debug) {
            console.log("q", data);
        }

        window[yieldr].fire(data);
    };
     ...
     * @type {Function}
     */
    window.ydResponse = window[yieldr].callback;
